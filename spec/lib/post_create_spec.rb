...
describe "Post Created" do
  ...
  before :all do
    open("http://#{HOST}/post_create?user=#{@user.id}&post=#{@post.id}&locale=#{@locale}&league=#{@league_id}&team=#{@team_id}&league_count=true&writers_count=true&ulb=false")
  end
  ...
  describe "LeagueWriters" do
    ...
    it "should not add 1 to the user who created the post if writers_count is false" do
      open("http://#{HOST}/post_create?user=#{@user.id}&post=#{@post.id}&league=#{@league_id}&team=#{@team_id + 1}&league_count=true&writers_count=false&locale=#{@locale}&ulb=false")
      @league_writers.set["user_#{@user.id}"].to_i.should == @league_writers.initial_set["user_#{@user.id}"].to_i + 3  # greater by 3 because of previous spec who does another call
    end
  end
  ...
  describe "TeamCounters" do
    ...
    describe "writers" do
      ...
      it "should increase the number of writers for the team if its a new writer" do
        open("http://#{HOST}/post_create?user=#{@user.id.to_i + 1}&post=#{@post.id}&league=#{@league_id}&team=#{@team_id}&league_count=true&writers_count=true&locale=#{@locale}&ulb=false")
      ...
      it "should not increase the number of original writers if ulb parameter is false " do
        open("http://#{HOST}/post_create?user=#{@user.id.to_i + 3}&post=#{@post.id}&league=#{@league_id}&team=#{@team_id}&league_count=true&writers_count=true&locale=#{@locale}&ulb=false")
        @team_counters.data["original_writers"].to_i.should eq @current_data["original_writers"].to_i + 1
      end
  ...
  describe "TeamWriters" do
    ...
    it "should not add 1 to the user who created the post if writers_count is false" do
      open("http://#{HOST}/post_create?user=#{@user.id}&post=#{@post.id}&league=#{@league_id}&team=#{@team_id}&league_count=true&writers_count=false&locale=#{@locale}")
      @team_writers.set["user_#{@user.id}"].to_i.should == @team_writers.initial_set["user_#{@user.id}"].to_i + 3  # greater by 3 because of previous spec who does another call
    end
  end
